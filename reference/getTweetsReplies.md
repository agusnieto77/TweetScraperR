...
**Obsoleta**: preféri getTweetsRepliesAPI(), basada en la API de X
(datos del JSON, mas robusta).

Esta función recupera las respuestas a un tweet específico en Twitter
(ahora X), incluyendo datos como el texto del tweet, usuario, fecha, y
URL. Utiliza web scraping para acceder a la página del tweet, iniciar
sesión con las credenciales proporcionadas, y recolectar la información
...
4.  Continuar scrolling y recolectando datos hasta alcanzar el número
    deseado o no encontrar nuevas respuestas.
...
- n_tweets:
  ...
  El número máximo de tweets de respuestas a recuperar. Por defecto es
  100.
...
Un data frame que contiene información sobre las respuestas al tweet
