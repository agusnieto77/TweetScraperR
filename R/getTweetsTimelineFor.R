...
#' @param kill_system Booleano que indica si se debe cerrar el navegador (solo las sesiones propias del paquete) después de la recolección (por defecto: FALSE).
#'
#' @details
...
getTweetsTimelineFor <- function(
  ...
  # Inicializar lista para acumular resultados (se combinan una sola vez al final)
  results <- vector("list", length(usernames))
  ...
  for (i in seq_along(usernames)) {
    ...
    tryCatch({
      ...
      # Acumular en la lista de resultados (se combinan al final)
      if (!is.null(user_tweets)) {
        results[[i]] <- user_tweets
      }
