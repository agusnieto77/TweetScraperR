...
#'
#' @param img_url Un vector de caracteres que contiene las URLs de las imágenes a analizar.
...
getTweetsImagesAnalysis <- function(img_url, modelo = "gpt-4o-mini", api_key = Sys.getenv("OPENAI_API_KEY"), dir = getwd()) {
  if (api_key == "") {
  ...
  img_url <- unlist(img_url)
  img_url <- img_url[!is.na(img_url)]
  ...
  3. **Texto contenido**: Si la imagen incluye texto, escribe el texto que aparece. De lo contrario, deja este campo en blanco.\n\n
  ...
  analyzeImage <- function(img_url) {
    ...
    result_json <- jsonlite::fromJSON(parsed_content$choices[[1]]$message$content, simplifyVector = TRUE)
    result_tibble <- tibble::as_tibble(result_json)
    
    result_tibble |>
      dplyr::mutate(url = img_url)
  }
  
  message("\nAnalizando las imágenes...\n")
  
  results <- purrr::map_dfr(img_url, analyzeImage, .progress = TRUE)
