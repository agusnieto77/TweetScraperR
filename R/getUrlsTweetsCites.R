...
#' getUrlsTweetsCites(url = "https://x.com/Picanumeros/status/1610715405705789442", n_urls = 130, save = FALSE)
#' }
#'
#' @references
...
getUrlsTweetsCites <- function(
  ...
  if (length(tweets_urls) > 0) {
    ...
    if (save) {
      saveRDS(tweets_urls, paste0(dir, "/cites_", sub("https://x.com/(.*)/status/(.*)", "\\1_\\2", url), "_", gsub("-|:|\\.", "_", format(Sys.time(), "%Y_%m_%d_%X")), ".rds"))
