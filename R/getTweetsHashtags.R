...
#' @return Un dataframe con una nueva columna 'hashtags' que contiene, para cada tweet,
#'   un vector de caracteres con sus hashtags. Los tweets sin hashtags devuelven
#'   `character(0)`. Nota: en versiones anteriores los hashtags llegaban anidados en
#'   una lista adicional (`x[[i]][[1]]`) y los tweets sin hashtags devolvían `NA`.
#' 
#' @export
#'
#' @examples
...
getTweetsHashtags <- function(df) {
  ...
  extract_single <- function(text) {
    ...
    # Si no hay hashtags, devolver un vector de caracteres vacío
